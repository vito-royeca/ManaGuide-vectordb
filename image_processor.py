...
class ImageProcessor:
    ...
    def crop_image(self, box_corners=None):
        if box_corners is not None and len(box_corners) == 4:
            ...
            for box in box_corners:
                box_corners = box[0]
                center_x = int(box_corners[:, 0].mean()) #Sum all the X coordinates and get the average for the center X
                center_y = int(box_corners[:, 1].mean()) # Do the same for the Y

                width = int(box_corners[:, 0].max() - box_corners[:, 0].min()) #Find the two points most opposite on the X axis, and subtract them to get the width
                height = int(box_corners[:, 1].max() - box_corners[:, 1].min()) #Do the same for the Y
            ...
            # Perform the cropping and perspective transformation
    ...
    def trim_image(self):
        ...
        #By now we have just the inside of our markers, it's time to find the card and essentially do the same thing
        ...
        ret, thresh = cv2.threshold(grayed, 130, 255, 0) #130 is the threshold value, 0-255, I found it via experimentation
